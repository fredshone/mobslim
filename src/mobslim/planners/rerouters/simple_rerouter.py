...
class StaticRouter(BaseRouter):
    def __init__(
        ...
        for edge in self.G.edges:
            ...
            min_duration = length / freespeed
            self.G[edge[0]][edge[1]]["minimum_duration"] = min_duration
        self.expectations = expectations
        for edge in self.G.edges:
            self.G[edge[0]][edge[1]]["expected_duration"] = expectations.get(
                edge, None
            )
    ...
    def update(
        ...
        for edge in self.G.edges:
            self.G[edge[0]][edge[1]]["expected_duration"] = (
                self.expectations.get(edge, None)
            )
    ...
    def get_route(self, source, target, time):
        ...
        minimum_durations = [
            self.G[u][v]["minimum_duration"] for u, v in link_ids
        ]
